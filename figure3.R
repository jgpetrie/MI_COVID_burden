...
deaths <- fread(paste0(pfp,"/burden_region_deaths.csv"),
                stringsAsFactors = FALSE) %>%
  mutate(Week.Ending.Date = as.Date(Week.Ending.Date)) %>%
  filter(Week.Ending.Date <= "2021-11-13")

hosp <- fread(paste0(pfp,"/burden_region_hosp.csv"),
              stringsAsFactors = FALSE) %>%
  mutate(Week.Ending.Date = as.Date(Week.Ending.Date)) %>%
  filter(Week.Ending.Date <= "2021-11-13")

pop <- fread(paste0(pfp,"/estimated_population_by_age.csv"),
             stringsAsFactors = FALSE) %>% 
...
plot.data.same <- plot.data %>%
  mutate(
    ...
    wave = case_when(Week.Ending.Date>=as.Date("2020-10-11") &
...
final.format <- function(data,grp){
  ...
  data2 <- bind_rows(case.plot.data,hosp.plot.data,death.plot.data) %>%
    mutate(
      variable = as.factor(case_when(grepl("adjLower",variable) ~ "cases lower",
...
plot.data.final.var[[2]]$lineage <- factor(plot.data.final.var[[2]]$lineage ,
...
###RESULTS text
ratios <- plot.data.final.var[[2]] %>%
  filter(Area=="Michigan" & grepl("Spring",lineage)) %>%
  group_by(lineage, AgeGroup) %>%
  summarise(case = sum(value))

ratios <- merge(filter(ratios,lineage=="Spring 2021: Alpha"),
                filter(ratios,lineage=="Spring 2021: Non Variant"),
                by = "AgeGroup") %>%
  mutate(ratio = case.x/(case.x+case.y))

###Plot
p1 <- ggplot(
  plot.data.final.var[[2]][plot.data.final.var[[2]]$Area == "Michigan", ],
  aes(x=mid.point.age,y = burden_per_100k,group = variable,fill=forColor)
  ) + 
  geom_area() +
  facet_wrap(~ lineage) +
  scale_fill_manual(
    values = c(c("lightsteelblue1","lightsteelblue3","lightsteelblue4"),
               c("firebrick1","firebrick3","firebrick4"),
               c("palegreen1","palegreen3","palegreen4")),
    name = "Lineage (outcome)") + 
  scale_y_continuous(breaks=seq(0,50000,10000), limits = c(0,50000)) +
  scale_x_continuous(breaks=c(9,18,25,35,45,55,65,75,85)) +
  theme_classic() +
  ylab("Burden per 100k population") + 
